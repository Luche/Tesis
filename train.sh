python train.py '../liputan6/data_dir' \
  --task bert_xymasked_wp_seq2seq -s de -t en --reset-optimizer \
  -a bert2bert --train-from-scratch --no-noisy-source --save-interval-updates 1000 \
  ...
  --weight-decay 0.0 --max-tokens 2000 --update-freq 1 --max-update 200000 \
  --left-pad-source False --adapter-dimension 512 --max-source-positions 512 --max-target-positions 512 \
  --use-adapter-bert --bert-model-name indolem/indobert-base-uncased --decoder-bert-model-name indolem/indobert-base-uncased