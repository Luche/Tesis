...
from .file_utils import cached_path, is_remote_url, hf_bucket_url, WEIGHTS_NAME, CONFIG_NAME
...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    'indobenchmark/indobert-base-p2': "https://s3.amazonaws.com/models.huggingface.co/bert/indobenchmark/indobert-base-p2",
    'indolem/indobert-base-uncased': "https://s3.amazonaws.com/models.huggingface.co/bert/indolem/indobert-base-uncased",
}
...
class BertSelfAttention(nn.Module):
    ...
    def forward(        
        ...
        # print("Shapes: ", query_layer.shape, key_layer.shape)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        print(input_ids.device, token_type_ids.device, position_ids.device, past_key_values_length.device)
        # Calculate embedding
        ...
        if attention_mask.dim() == 3:
            ...
            # - if the model is a decoder, apply a causal mask in addition to the padding mask
            # - if the model is an encoder, make the mask broadcastable to [batch_size, num_heads, seq_length, seq_length]
            if self.is_decoder and self.is_at:
                batch_size, seq_length = input_shape
                seq_ids = torch.arange(seq_length, device=device)
                causal_mask = seq_ids[None, None, :].repeat(batch_size, seq_length, 1) <= seq_ids[None, :, None]
                # in case past_key_values are used we need to add a prefix ones mask to the causal mask
        ...
        # if encoder_hidden_states is not None:
            ...
            # extended_attention_mask = extended_attention_mask.transpose(0, 3)
            # print("Embedding: ", embedding_output.shape)
            # print("Encoder hidden states: ", encoder_hidden_states.shape)
            # print("Attention mask: ", extended_attention_mask.shape)
        ...
        if self.is_encoder and not self.is_at:
            ...
            encoded_layers = sequence_output[:, 1:, :]
