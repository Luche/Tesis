...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        # Load config
        # if 'indolem' in serialization_dir:
        config_file = os.path.join(serialization_dir, CONFIG_NAME)
        # else:
        #     config_file = serialization_dir + '-' + CONFIG_NAME
        r = requests.get(config_file, allow_redirects=True)
        open(os.path.join(tempdir, CONFIG_NAME), 'wb').write(r.content)
        config_file = os.path.join(tempdir, CONFIG_NAME)
        ...
        if state_dict is None and not from_tf:
            ...
            logger.info("Downloading weight from {}".format(weights_path))
            r = requests.get(weights_path, allow_redirects=True)
            open(os.path.join(tempdir, WEIGHTS_NAME), 'wb').write(r.content)
            weights_path = os.path.join(tempdir, WEIGHTS_NAME)
...
class BertModelEncoder(BertModel):
    def __init__(self, config):
        super(BertModelEncoder, self).__init__(config)
        self.encoder = BertEncoder(config)
        self.embed_lengths = nn.Embedding(1024, self.hidden_size)
        nn.init.normal_(self.embed_lengths.weight, mean=0, std=0.02)
