...
class SequenceGeneratorWithBert(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        # if self.decode_use_adapter:
        #     bert_encoder_padding_mask = src_tokens.eq(model.models[0].berttokenizer.pad())
        #     bert_encoder_out, _, predicted_lengths = model.models[0].encoder(src_tokens, attention_mask=1-bert_encoder_padding_mask.float())
        #     bert_encoder_out = bert_encoder_out.permute(1,0,2).contiguous()
        #     if model.models[0].mask_cls_sep:
        #         bert_encoder_padding_mask += bert_input.eq(model.models[0].berttokenizer.cls())
        #         bert_encoder_padding_mask += bert_input.eq(model.models[0].berttokenizer.sep())
        #     encoder_outs = [{
...
class EnsembleModel(torch.nn.Module):
    ...
    def _decode_one(
        ...
        decoder_out = list(model.decoder(tokens, src_tokens=src_tokens, encoder_out=encoder_out))
        if temperature != 1.:
